...
if (pthread_create(&thread4, NULL, &func, &info4) != 0) {
   ...
   printf("Exec time for thread1(add) = %lf nsec\n", info1.exec_time_1);
   
//
   pthread_join(thread2, NULL);
   printf("Exec time for thread2(sub) = %lf nsec\n", info1.exec_time_2);
  

   pthread_join(thread3, NULL);
   printf("Exec time for thread3(mul) = %lf nsec\n", info1.exec_time_3);
   

   pthread_join(thread4, NULL);

   printf("Exec time for thread4(div) = %lf nsec\n", info1.exec_time_4);
   pthread_exit(NULL);
